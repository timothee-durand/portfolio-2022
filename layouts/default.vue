  ...
  <div
    ...
    :class="{
      ...
      'is-content-transition':
        !keepTransitionElement && isTransitioning && !isDefaultTransition,
    }"
  >
    ...
    leave(el, done) {
      try {
        const lastLinkClicked =
          this.$store.state.transitionComponent &&
          this.$store.state.transitionComponent.closest('a')
        let svg
        if (!lastLinkClicked) {
        ...
        if (!this.isDefaultTransition) {
          ...
          if (
            lastLinkClicked &&
            !lastLinkClicked.classList.contains('header-link')
          ) {
