...
export default {
  ...
  methods: {
    ...
    update() {
      ...
      if (!this.isPhone) {
        ...
        this.camera.position.x +=
          (-this.pointer.x - this.camera.position.x) * 0.05
        this.camera.position.y +=
          (-this.pointer.y - this.camera.position.y) * 0.05
      } else {
        //if phone / tablet / ...
    ...
    loadTexture(directoryName, fileName) {
      ...
      return new Promise((resolve) => {
        this.textureLoader.load(
          baseTexturePath + directoryName + '/' + fileName,
          (texture) => {
            resolve(texture)
          }
        )
    ...
    async loadTextures() {
      ...
      textures.eclair2 = await this.loadTexture(
        particlesSpritesDirName,
        'eclair-2.png'
      )
      textures.circle1 = await this.loadTexture(
        particlesSpritesDirName,
        'circle-1.png'
      )
      textures.circle2 = await this.loadTexture(
        particlesSpritesDirName,
        'circle-2.png'
      )
    ...
    addParticles() {
      ...
      const possibleRotations = [0, Math.PI / 2, Math.PI / 3]
